 ...
 */
class OwnEval(target: Option[File]) {
  /**
  ...
  def applyProcessed[T](className: String, code: String, resetState: Boolean): T = {
    // Change JU!!!!
    Thread.currentThread().asInstanceOf[OwnPrintStreamThread].startCompilation()
    val cls = compiler(wrapCodeInClass(className, code), className, resetState)
    Thread.currentThread().asInstanceOf[OwnPrintStreamThread].stopCompilation()

    Thread.currentThread().asInstanceOf[OwnPrintStreamThread].startExecuting()
    val result = cls.getConstructor().newInstance().asInstanceOf[() => Any].apply().asInstanceOf[T]
    Thread.currentThread().asInstanceOf[OwnPrintStreamThread].stopExecuting()
      ...
      synchronized {
        ...
        findClass(className).getOrElse {
          try {
            ...
            case e : Exception =>
              System.err.println(e.getMessage) // Change JU!!!!
              Thread.currentThread().asInstanceOf[OwnPrintStreamThread].stopExecutingWithoutTime()
              null
          }
        }
      }
